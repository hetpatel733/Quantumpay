...
const PaymentLinkModal = ({ isOpen, onClose, onSuccess, userData }) => {
    ...
    try {
      ...
      if (apiKeysResponse.success) {
        ...
        const normalized = allKeys.map((k, idx) => ({
          ...
          key:
            k.key || k.value || k.apiKey || k._id?.toString() || `key-${idx}`,
        ...
        //console.log(
        //`✅ Found ${activeKeys.length} active API keys out of ${normalized.length} total`
        //);
  ...
  const generatePaymentLink = () => {
    ...
    // Debugging info
    //console.log("🔎 generatePaymentLink inputs:", {
      //selectedItem,
      //selectedApiKey,
    //});
    //console.log(
      //"📦 portfolioItems sample:",
      //portfolioItems && portfolioItems.length ? portfolioItems[0] : "no-items"
    //);
    //console.log(
    //  "🔑 apiKeys sample:",
    //  apiKeys && apiKeys.length ? apiKeys[0] : "no-keys"
    //);
    // Try to find the order by several possible id fields
    const selectedOrder = portfolioItems.find(
      (item) =>
    ...
    const selectedApi = apiKeys.find(
      (key) =>
    ...
    if (!selectedOrder || !selectedApi) {
      console.warn("⚠️ Invalid selection details:", {
        ...
        portfolioItemIds: portfolioItems.map((p) => ({
          _id: p._id,
          id: p.id,
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)] bg-white dark:bg-gray-800">
          {loading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-3 text-text-secondary dark:text-gray-400">
                Loading...
              </span>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div>
                ...
                {portfolioItems.length === 0 && (
                  <p className="text-sm text-text-secondary dark:text-gray-400 mt-1">
                    No products found. Create a product in Portfolio Management
                    first.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                >
                  ...
                  {apiKeys.map((keyObj, idx) => (
                    <option
                      key={keyObj._id || `apikey-${idx}`}
                      value={keyObj._id}
                      className="dark:bg-gray-900"
                    >
                      {keyObj.label} (
                      {keyObj.key?.substring(0, 10) ||
                        keyObj._id.substring(0, 10)}
                      ...)
                    </option>
                  ))}
                </select>
              ...
              {generatedLink && (
                <div className="bg-success-50 dark:bg-green-900/20 border border-success-200 dark:border-green-800/50 rounded-lg p-4">
                  <h3 className="text-sm font-medium text-success-700 dark:text-green-300 mb-2">
