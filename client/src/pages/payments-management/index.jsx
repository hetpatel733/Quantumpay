...
const MockModal = ({ title, onClose, children }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div className="bg-surface dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md relative border border-border dark:border-gray-700">
      ...
      <button
        onClick={onClose}
        className="absolute top-3 right-3 p-2 rounded-full hover:bg-secondary-100 dark:hover:bg-gray-700"
      >
        <Icon name="X" size={20} />
      </button>
      {children}
...
const PaymentsManagement = ({ userData }) => {
      ...
      try {
        ...
        if (response.success) {
          ...
          const transformedPayments = paymentsData.map((payment) => ({
            ...
            cryptoType:
              payment.cryptoType || payment.cryptoCurrency || "Unknown",
            cryptoSymbol:
          ...
          setStats({
            ...
            completed: transformedPayments.filter(
              (p) => p.status === "completed"
            ).length,
            pending: transformedPayments.filter((p) => p.status === "pending")
              .length,
            failed: transformedPayments.filter((p) => p.status === "failed")
              .length,
          });
        ...
        if (err.message.includes("403") || err.message.includes("401")) {
          ...
          setError(
            "Payment processing is currently paused. Please contact support to reactivate your account."
          );
  ...
  const filteredAndSortedPayments = useMemo(() => {
    ...
    if (filters.cryptocurrency !== "all") {
      filtered = filtered.filter(
        (p) => p.cryptoType === filters.cryptocurrency
      );
    }
  ...
  const handleViewDetails = (payment) => {
    ...
    navigate(
      `/dashboard/payment-details-modal?id=${payment.payId || payment.id}`
    );
  };

  const handleCreatePaymentLink = () => setIsPaymentLinkModalOpen(true);
  const handleBulkExport = () =>
    showToast(`Exporting ${selectedPayments.length} payments...`, "info");
  ...
  if (loading)
    return (
      <div className="text-center p-12 dark:bg-gray-900 dark:text-white min-h-screen flex items-center justify-center">
        Loading payments...
      </div>
    );
  if (error)
    return (
      <div className="text-center p-12 text-error dark:bg-gray-900 min-h-screen flex items-center justify-center">
        {error}
      </div>
    );
      ...
      <div>
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4">
          <div>
            ...
            <p className="text-text-secondary dark:text-gray-400 mt-1">
              Monitor and manage all cryptocurrency transactions.
            </p>
          </div>
          <div className="flex items-center space-x-3 mt-4 md:mt-0">
            {selectedPayments.length > 0 && (
              <button
                ...
                className="flex items-center space-x-2 px-4 py-2 bg-secondary-100 dark:bg-gray-700 text-text-primary dark:text-white rounded-lg hover:bg-secondary-200 dark:hover:bg-gray-600"
              >
            ...
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 bg-primary dark:bg-teal-500 text-white rounded-lg hover:bg-primary-700 dark:hover:bg-teal-600"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-surface dark:bg-gray-800 rounded-lg p-4 border border-border dark:border-gray-700">
            ...
            <p className="text-2xl font-semibold text-error">{stats.failed}</p>{" "}
          </div>
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-border dark:divide-gray-700">
            <thead className="bg-background dark:bg-gray-900">
              <tr>
                ...
                <th
                  className="px-6 py-3 text-left text-xs font-medium text-text-secondary dark:text-gray-400 uppercase tracking-wider cursor-pointer"
              ...
              {paginatedPayments.length > 0 ? (
                paginatedPayments.map((payment) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <input
                        ...
                        checked={selectedPayments.includes(
                          payment.payId || payment.id
                        )}
                        onChange={() =>
                          handleSelectPayment(payment.payId || payment.id)
                        }
                        className="dark:bg-gray-700 dark:border-gray-600"
                      />
                    </td>
                    ...
                    <td className="px-6 py-4">
                      {getStatusComponent(payment.status)}
                    </td>
                ...
                <tr>
                  <td
                    colSpan="8"
                    className="text-center py-12 dark:text-gray-400"
                  >
          ...
          <div className="px-6 py-4 flex items-center justify-between border-t border-border dark:border-gray-700">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="px-3 py-1 rounded-lg text-sm disabled:opacity-50 dark:text-white dark:hover:bg-gray-700"
              >
                Previous
              </button>
              <button
